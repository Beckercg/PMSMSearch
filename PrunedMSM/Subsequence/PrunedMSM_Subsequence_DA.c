...
double msmDistPruned(double *X, double *Y, int m, double *tmpArray, double *upperBoundArray)
{
...
int main(  int argc , char *argv[] )
{
        ...
        } else
        {
            ...
            for(i=0; i<ep; i++)
            {
                ...
                if( i >= m-1 )
                {
                    ...
                    distCalc = msmDistPruned(tz,q,m,tmpArray, upperBoundArray);
                    if( distCalc < bsf )
                    {   /// Update bsf
                        bsf = distCalc;
    ...
    fprintf(rd,"%s,%i,%lli,%f,%lld,%f\n", "PrunedMSM_DA", m,i,bsf,loc, (t2-t1)/CLOCKS_PER_SEC);
