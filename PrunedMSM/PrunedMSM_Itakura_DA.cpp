...
double msmDistPruned(double *X, double *Y, int m, int n, double slope)
{
    ...
    for (i = 1; i < m; i++)
    {
        double start_1, start_2, end_1, end_2;
        ...
        //TODO does not work
        start_1 = 1*slope;
        start_2 = 1/slope*i-(1-slope)/slope*m;
        end_1 = 1/slope*i;
        end_2 = slope*i+(1-slope)*m;

        int start_j = (int)ceil(max(start_1 , start_2));
        int end_j = (int)ceil(min(end_1 , end_2));
        ...
        tmpArray[0] =INF;
        for(k=0; k<start_j; k++)    tmpArray[k]=INF;
        for(k=end_j+1; k<=m; k++)    tmpArray[k]=INF;
...
int main(  int argc , char *argv[] )
{
    ...
    fprintf(rd,"%s%s,%s,%f,%f\n", "PrunedMSM_Itakura_DA_", argv[5],dataset.c_str(),acc, (t2-t1)/CLOCKS_PER_SEC);
    fclose(rd);
    return 0;
}
