...
double lb_cost_l(double *t, double *q, int len, double bsf)
{
    double lb;
    int mvcount=0;
...
int main(  int argc , char *argv[] )
{
    ...
    for (int i = 0; i < query_size; i++){
        ...
        for (int j = 0; j < sequence_size; j++){
            ...
            lb_edcost = lb_cost_l(q_file[i], s_file[j], m, bsf);
            if(lb_edcost < bsf){
                if(distance < bsf)
                {
                    bsf = distance;
    ...
    fprintf(rd,"%s,%s,%f,%f\n", "PMSMSearch with LB_CostL",dataset,acc, (t2-t1)/CLOCKS_PER_SEC);
    fclose(rd);
    return 0;
}
