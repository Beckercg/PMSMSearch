...
double msmDistPruned(double *X, double *Y, int m, int n, double bsf, double slope, double *tmpArray, double *upperBoundArray, double *ts1, double *ts2)
{
...
int main(  int argc , char *argv[] )
{
    ...
    for (int i = 0; i < query_size; i++){
        ...
        for (int j = 0; j < sequence_size; j++){
            distance = msmDistPruned(q_file[i], s_file[j], m, m, bsf, slope, tmpArray, upperBoundArray, ts1, ts2);
            if(distance < bsf)
            {
                bsf = distance;
                bclass = sclass[j];
