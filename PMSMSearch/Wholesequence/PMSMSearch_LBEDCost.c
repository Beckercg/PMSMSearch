...
double lb_edcost(double *t, double *q, int len, double bsf)
{
    double lb;
...
int main(  int argc , char *argv[] )
{
    ...
    for (int i = 0; i < query_size; i++){
        ...
        for (int j = 0; j < sequence_size; j++){
            ...
            glb = lb_edcost(q_file[i], s_file[j], m, bsf);
            if(glb < bsf){
                if(distance < bsf)
                {
                    bsf = distance;
    ...
    fprintf(rd,"%s,%s,%f,%f,%d\n", "PMSMSearch with LB_EDCost",dataset,acc, (t2-t1)/CLOCKS_PER_SEC, lb_count);
    fclose(rd);
    return 0;
}
