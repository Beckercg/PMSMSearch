 ...
 */
double msmDistPruned(const vector<double> &X, const vector<double> &Y, const bool &bsf)
{
...
int main(  int argc , char *argv[] )
{
    ...
    fprintf(rd,"%s,%s,%d,%d,%f,%f\n", "PMSMSearch",dataset.c_str(), queryfile.size(), queryfile[0].size(), sequencefile.size(), sequencefile[0].size(),acc, (t2-t1)/CLOCKS_PER_SEC);
    return 0;
}
