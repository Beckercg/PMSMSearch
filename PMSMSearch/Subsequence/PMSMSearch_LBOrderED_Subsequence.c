...
*/
double lb_sakoe_tree(Index *t, Index *q, int len, double bsf)
{
...
int main(  int argc , char *argv[] )
{
        ...
        } else
        {
            ...
            for(i=0; i<ep; i++)
            {
                ...
                if( i >= m-1 )
                {
                    ...
                    global_lb = lb_sakoe_tree(T_tmp, Q_tmp,  m, bsf);
                    if (global_lb < bsf)
                    {
    ...
    fprintf(rd,"%s,%i,%lli,%f,%lld,%d,%f\n", "PMSMSearch with LB_Sorted", m,i,bsf,loc,sakoetree, (t2-t1)/CLOCKS_PER_SEC);
    fclose(rd);
